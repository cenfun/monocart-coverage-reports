...
const showBranchesNote = (target, visible) => {

    if (visible) {
        state.branchesNoteTarget = target;
        state.branchesNoteVisible = true;
        return;
    }

    state.branchesNoteVisible = false;
...
<template>
  <div class="mcr vui-flex-column">
    ...
    <VuiPopover
      v-model="state.branchesNoteVisible"
      :target="state.branchesNoteTarget"
      width="370px"
    >
      <IconLabel
        icon="warning"
      ...
      <div class="mcr-note">
        The result of branch coverage is generated by calculating the source code <a
