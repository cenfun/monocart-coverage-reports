...
const executeCommand = async (command, cliOptions) => {
    ...
    const outputDir = coverageReport.options.outputDir;
    const nodeV8CoverageDir = Util.relativePath(path.resolve(outputDir, '.v8-coverage'));
    ...
    const subprocess = foregroundChild(command, {
        ...
        const onReady = cliOptions.onReady;
