...
const getRequirePath = (filename) => {
    return quoteNodeOption(path.resolve(__dirname, 'register', filename));
};

const getImportPath = (filename) => {
    return quoteNodeOption(pathToFileURL(path.resolve(__dirname, 'register', filename)).href);
};
...
const initNodeOptions = async (cliOptions) => {
    ...
    if (preloadType === '--import') {
        const importPath = getImportPath('register.mjs');
        nodeOptions.unshift(`--import ${importPath}`);
    } else {
        const requirePath = getRequirePath('register.js');
