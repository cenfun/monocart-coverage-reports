...
const {
    getCoverageDataList, generateCoverageReports, findHtmlPath
} = require('./generate.js');
...
class CoverageReport {
    ...
    initOptions() {
        ...
        // cache dir name in the output dir
        const cacheDirName = `${this.options.cacheDirName || defaultOptions.cacheDirName}`;
        this.options.cacheDirName = cacheDirName;
    ...
    async generate() {
        ...
        fs.readdirSync(outputDir).forEach((itemName) => {
            if (itemName === this.options.cacheDirName) {
                return;
            }
            Util.rmSync(path.resolve(outputDir, itemName));
        });

        const report = await generateCoverageReports(dataList, this.options);

        // find first index.html path for preview
        const htmlPath = findHtmlPath(outputDir);
        report.htmlPath = Util.relativePath(htmlPath);
