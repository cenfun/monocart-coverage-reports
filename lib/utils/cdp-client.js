...
class CoverageClient {
    ...
    bindEvents(events) {
        if (events) {
            Object.keys(events).forEach((eventType) => {
                this.session.on(eventType, events[eventType]);
            });
        }
    }
    ...
    unbindEvents(events) {
        if (events) {
            Object.keys(events).forEach((eventType) => {
                this.session.off(eventType, events[eventType]);
            });
        }
    }
    ...
    async stopJSCoverage() {
        ...
        this.unbindEvents(this.jsEvents);
        this.jsEvents = null;
        ...
        jsCoverage.forEach((entry) => {
            // add anonymous url
            if (!entry.url) {
                entry.url = `anonymous-${anonymousIndex++}.js`;
            }
            // add source
            const source = this.scriptSources.get(entry.scriptId);
            if (!source) {
                Util.logDebug(`Not found source: ${entry.url}`);
                return;
            }
            entry.source = source;
        });
    ...
    async startCSSCoverage() {
        ...
        this.cssEvents = {
            'CSS.styleSheetAdded': (e) => {
                ...
                let url = sourceURL;
                if (!url) {
                    url = `anonymous-${anonymousIndex++}.css`;
                }
        ...
        this.bindEvents(this.cssEvents);
    ...
    async stopCSSCoverage() {
        ...
        this.unbindEvents(this.cssEvents);
        this.cssEvents = null;
...
module.exports = async (cdpOptions) => {

    const options = initOptions(cdpOptions);
    ...
    return new Promise((resolve, reject) => {
        ...
        const client = new CoverageClient(session);
