...
const Util = {
    ...
    ensureDirSync: function(filePath) {
    ...
    strToObj: (str) => {
        ...
        // Accept JSON-like objects using single or double quotes, e.g.
        //   {'**/node_modules/**':false,'**/src/*.js':true}
        // Normalize single quotes to double quotes, then JSON.parse (safe, no code execution).
        ...
        try {
            ...
            // not a parseable object literal
        }
    },
    ...
    createFilterHandler: function(input, fieldAccessor) {
        ...
        if (input && typeof input === 'string') {
            ...
            if (obj) {
                ...
                return (item) => {
                    return Util.betterMinimatch(fieldAccessor(item), input);
                };
            }
        }
        ...
        if (input && typeof input === 'object') {
            ...
            return (item) => {
                const value = fieldAccessor(item);
                for (const pattern of patterns) {
                    if (Util.betterMinimatch(value, pattern)) {
    ...
    getEntryFilter: (options) => {
        ...
        const input = options.entryFilter || options.filter;
        const handler = Util.createFilterHandler(input, (entry) => entry.url);
    ...
    getSourceFilter: (options) => {
        ...
        const input = options.sourceFilter || options.filter;
        const handler = Util.createFilterHandler(input, (sourcePath) => sourcePath);
    ...
    fixSourceRange: (locator, start, end) => {
        ...
        const fixedStart = start + startOffset;
