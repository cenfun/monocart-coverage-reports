...
const getSourcePath = (url, basePath, index = '', type = '') => {
    ...
    if (url.startsWith('file:')) {
        const relPath = Util.relativePath(fileURLToPath(url), basePath);
...
const initSourceMapSourcePath = (sourceMap, fileUrls, distFile, sourcePathHandler, basePath) => {

    sourceMap.sources = sourceMap.sources.map((sourceName, i) => {

        const sourceUrl = resolveSourceUrl(sourceName, sourceMap.sourceRoot);

        let sourcePath = getSourcePath(sourceUrl, basePath, i + 1);
        if (sourcePathHandler) {
