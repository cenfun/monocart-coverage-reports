...
const calculatePctAndStatus = (item, watermarks) => {
    Object.keys(item).forEach((k) => {
        const it = item[k];
        it.uncovered = it.total - it.covered;
        it.pct = Util.PNF(it.covered, it.total, 2);
        it.status = Util.getStatus(it.pct, watermarks[k]);
    });
};
...
const saveV8Report = async (v8list, options) => {
    ...
    summaryList.forEach((item) => {
        ...
        Object.keys(item).forEach((k) => {
            summary[k].total += item[k].total;
            summary[k].covered += item[k].covered;
        });
    });
