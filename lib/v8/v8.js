...
const getUncoveredLines = (file, cdOptions) => {
    const lines = [];
    ...
    const addLines = () => {
        if (startLine) {
            if (endLine) {
                lines.push(startLine + EC.red('-') + endLine);
                startLine = null;
                endLine = null;
            } else {
...
const getGroupedRows = (files, metrics, cdOptions) => {
    ...
    files.forEach((file) => {
        ...
        fileRow.uncoveredLines = getUncoveredLines(file, cdOptions);
