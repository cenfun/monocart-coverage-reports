...
const initV8ListAndSourcemap = async (mcr, v8list) => {
    ...
    for (const item of sourceList) {
        ...
        await Util.writeFile(item.sourcePath, JSON.stringify(item.sourceData));

        // save source and sourcemap file for debug
        // https://evanw.github.io/source-map-visualization
        // if (item.sourceData.sourceMap) {
        //     await Util.writeFile(`${item.sourcePath}.js`, item.sourceData.source);
        //     await Util.writeFile(`${item.sourcePath}.js.map`, JSON.stringify(item.sourceData.sourceMap));
        // }
    }
