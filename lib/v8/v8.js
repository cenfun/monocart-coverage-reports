...
const calculatePctAndStatus = (item, watermarks) => {
    Object.keys(item).forEach((k) => {
        ...
        if (indicateData.total) {
            indicateData.pct = Util.PNF(indicateData.covered, indicateData.total, 2);
            indicateData.status = Util.getStatus(indicateData.pct, watermarks[k]);
        } else {
            indicateData.pct = '';
            indicateData.status = 'unknown';
        }
