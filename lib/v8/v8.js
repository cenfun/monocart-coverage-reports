...
const getEntryFilter = (options) => {
    ...
    const getFullyQualifiedPattern = (pattern) => {
        return `file://${path.join(basePath, pattern)}`;
    };
    ...
    if (input && typeof input === 'string') {
        ...
        if (obj) {
            ...
            return (entry) => {
                const fullyQualifiedPattern = getFullyQualifiedPattern(input);

                return minimatch(entry.url, fullyQualifiedPattern);
            };
        }
    }
    ...
    if (input && typeof input === 'object') {
        ...
        return (entry) => {
            ...
            for (const pattern of patterns) {
                const fullyQualifiedPattern = getFullyQualifiedPattern(pattern);
                if (minimatch(url, fullyQualifiedPattern)) {
