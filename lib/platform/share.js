const Util = {
    ...
    findInRanges: (startPos, endPos, rangeList, startKey = 'start', endKey = 'end') => {
        ...
        // rangeList is sorted by startKey ascending.
        // Binary search for the rightmost range where startKey <= startPos.
        let low = 0;
        let high = rangeList.length;
        while (low < high) {
            const mid = (low + high) >>> 1;
            if (rangeList[mid][startKey] > startPos) {
                high = mid;
            } else {
                low = mid + 1;
            }
        ...
        // Scan backwards from the insertion point; the first match is the answer.
