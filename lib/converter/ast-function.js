...
const getBranchBlock = (start, end, functionInfo) => {
...
const addNodeCount = (node, functionInfo) => {
...
const addBranch = (group, node, locationMap, functionInfo) => {
    ...
    const block = getBranchBlock(start, end, functionInfo);
...
const visitFunction = (functionInfo, outputInfo, functionNode) => {

    const { branchMap, locationMap } = outputInfo;

    Util.visitAst(functionNode, {

        Function(node) {
            if (node === functionNode) {
                return;
            }
            // break all sub functions
            return 'break';
        },
        ...
        BlockStatement: (node, parents) => {
            ...
            addNodeCount(node, functionInfo);
        ...
        AssignmentPattern: (node, parents) => {
            ...
            addBranch(group, node, locationMap, functionInfo);
        },
...
const getStatementBlock = (start, end, functionInfo) => {
...
const generateStatements = (statementNodes) => {
    ...
    statementNodes.forEach((item) => {
        ...
        const parent = reverseParents.find((it) => it._state && it._state.isFunction);
        if (parent) {
            const block = getStatementBlock(start, end, parent._state);
            if (block) {
