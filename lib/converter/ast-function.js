...
const getBlockRange = (node, functionRange) => {
    ...
    const { start, end } = node;
    const blockRange = blockMap.get(start);
    if (blockRange) {
        return blockRange;
    }
...
const getRootCount = (node, functionRoot) => {
    if (!functionRoot) {
        return 1;
    }
    ...
    // root function count
    return functionRoot.count;
};

const getNodeCount = (item, functionRoot) => {
    const { node, reverseParents } = item;
    if (node._state) {
        return node._state.count;
    }
    ...
    return getRootCount(node, functionRoot);
};

const generateStatements = (statementNodes, functionRoot) => {
    return statementNodes.map((item) => {
        const { node } = item;
        const { start, end } = node;
...
const visitFunctionParams = (functionInfo, branchMap, locationMap, params, parents) => {
...
const visitFunctionBody = (functionInfo, branchMap, locationMap, body) => {
