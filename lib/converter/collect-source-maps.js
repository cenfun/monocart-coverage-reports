...
const getEvalSourceMapData = (content, url) => {
    ...
    // try from eval sourcemap
    // could be multiple sourcemaps
    ...
    while (i < len) {
        ...
        if (item) {
            ...
            if (content[i] === '\n'
                || (content[i] === '\\' && content[i + 1] === 'n')
                || (content[i] === '/' && content[i + 1] === '/' && content[i + 2] === '#')) {
                const sourceMapComments = content.slice(item.start, i);

                let smc;
                try {
                    smc = convertSourceMap.fromComment(sourceMapComments);
                } catch (e) {
                    // console.log(e.message, sourceMapComments);
                }
                if (smc) {
                    item.end = i;
                    item.sourceMap = smc.sourcemap;
                    list.push(item);
                }
        ...
        const start = content.indexOf(key, i);
        if (start === -1) {
            break;
        }
    ...
    console.log(url, list.map((it) => it.sourceMap.file));

    // merge sourcemap?

};

const getInlineSourceMapData = (content, url) => {
    // try from file end comments
    let smc;
    try {
    ...
    if (smc) {
        return smc.sourcemap;
    }

    return getEvalSourceMapData(content, url);
};
...
const collectSourceMaps = async (v8list, options) => {
    ...
    for (const item of v8list) {
        ...
        if (type === 'js' && !sourceData.sourceMap) {
            // from file comments sync
            const sourceMapData = getInlineSourceMapData(source, url);
            if (sourceMapData) {
                sourceData.sourceMap = resolveSourceMap(sourceMapData, url);
