...
const handleFunctionBranches = (fNode, functionInfo, cache) => {
    ...
    Util.visitAst(fNode, {
        ...
        LogicalExpression(node) {
            ...
            const linkStart = addBranchNode('LogicalExpression', start, left, functionInfo, cache);
            addBranchNode('LogicalExpression', linkStart, right, functionInfo, cache);
        },
        ...
        ConditionalExpression(node) {
            ...
            const linkStart = addBranchNode('ConditionalExpression', start, consequent, functionInfo, cache);
            addBranchNode('ConditionalExpression', linkStart, alternate, functionInfo, cache);
        },

        IfStatement(node) {
            const { consequent, alternate } = node;
            const start = consequent.start;
            const linkStart = addBranchNode('IfStatement', start, consequent, functionInfo, cache);
            if (alternate) {
                addBranchNode('IfStatement', linkStart, alternate, functionInfo, cache);
            } else {
                addEmptyBranchNode(linkStart, consequent, functionInfo, cache);
            }
        }
