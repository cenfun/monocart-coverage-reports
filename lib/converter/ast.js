...
const findInRanges = (item, ranges) => {
    // TODO high performance find
    return ranges.find((range) => {
        return item.start >= range.startOffset && item.end <= range.endOffset;
    });
};
...
const handleFunctions = (functions, cache) => {
    ...
    functions.forEach((item) => {
        ...
        const uncoveredRange = findInRanges(item, uncoveredFunctionRanges);
        if (uncoveredRange) {
            item.count = 0;
            return;
        }
...
const handleBranches = (branches, cache) => {
    ...
    branches.forEach((branchInfo) => {
        ...
        branchInfo.locations.forEach((item, i) => {
            ...
            // support for tf2 || a || 1;
            // "a" is in "|| a"
            const blockRange = findInRanges(item, blockRanges);
            if (blockRange) {
                item.count = blockRange.count;
                return;
            }
