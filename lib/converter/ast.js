...
const collectAstInfo = (ast, countInfo) => {
    ...
    const { functionNodes, statementNodes } = collectNodes(ast);
    ...
    if (rootFunction) {
        ...
        visitFunction(functionInfo, outputInfo, ast);
    }
    ...
    functionNodes.forEach((item) => {
        ...
        visitFunction(functionInfo, outputInfo, node);
    });

    const { branchMap } = outputInfo;
