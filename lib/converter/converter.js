...
const initDistFileCoverage = (item, state) => {

    const coverage = getEmptyCoverage();
    ...
    if (type === 'js') {
        ...
        coverage.functionMap = functionMap;
...
const getDistCoverage = (item, state) => {
    ...
    // js file
    if (type === 'js') {
        const coverage = initDistFileCoverage(item, state);
...
const generateOriginalList = (item, state, originalMap) => {
...
const unpackSourceMap = async (item, state, options) => {
    ...
    // collect list
    state.sourceList = generateOriginalList(item, state, originalMap);
