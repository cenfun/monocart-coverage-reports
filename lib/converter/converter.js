...
const collectFileCoverage = (v8Data, state, options) => {
    ...
    if (ignoredRanges) {
        ...
        // statements is InfoStatement start/end/count instance
...
const handleOriginalFunctionsCoverage = (state, originalStateMap) => {
    ...
    state.astInfo.functions.forEach((it) => {
        ...
        const originalFunction = {
            ...
            bodyStart: originalRange.startOffset,
            bodyEnd: originalRange.endOffset
        };
        ...
        const bodyResult = findOriginalRange(bodyRange, state, originalStateMap);
        if (!bodyResult.error) {
            originalFunction.bodyStart = bodyResult.originalRange.startOffset;
            originalFunction.bodyEnd = bodyResult.originalRange.endOffset;
        }

        // add back to original ast
        originalState.astInfo.functions.push(originalFunction);
...
const handleOriginalBranchesCoverage = (state, originalStateMap) => {
    ...
    state.astInfo.branches.forEach((it) => {
        ...
        const newLocations = locations.map((oLoc) => {
            ...
            newLoc.generatedStart = start;
            newLoc.generatedEnd = end;
...
const handleOriginalStatementsCoverage = (state, originalStateMap) => {
    ...
    state.astInfo.statements.forEach((it) => {
        ...
        // add back to original ast
        originalState.astInfo.statements.push({
            ... it,
            generatedStart: start,
            generatedEnd: end,
            start: originalRange.startOffset,
    ...
    if (js) {
        ...
        coverageList.forEach((block) => {
            block.ranges.forEach((range, index) => {
                ...
                originalRange.generatedStart = range.startOffset;
                originalRange.generatedEnd = range.endOffset;
