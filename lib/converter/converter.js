...
const addJsCoverage = (state, range) => {

    const { locator, coverageInfo } = state;

    const { ranges, lineMap } = coverageInfo;
    ...
    lines.forEach((it) => {
        ...
        if (it.entire) {
            ...
            line.covered = false;
            line.count = Math.max(line.count, count);
        }
    ...
    // if (state.sourcePath.endsWith('branch.js')) {
...
const addCssCoverage = (coverageInfo, range, locator) => {
...
const handleFunctionsCoverage = (state) => {
    ...
    astInfo.functions.forEach((it) => {
        ...
        const loc = updateOffsetToLocation(locator, {
            start,
            end
        });
...
const handleBranchesCoverage = (state) => {
    ...
    astInfo.branches.forEach((it) => {
        ...
        const branchLoc = updateOffsetToLocation(locator, it.loc);

        // locations
        // [ { start:{line,column}, end:{line,column}, count }, ...]
        const locations = it.locations.map((loc) => {
            const newLoc = updateOffsetToLocation(locator, loc);
...
const handleRangesCoverage = (state) => {
    ...
    const {
        js, locator, coverageInfo, coverageList
    } = state;

    if (js) {
        coverageList.forEach((block) => {
            block.ranges.forEach((range, index) => {
                addJsCoverage(state, range);
            });
        });

    } else {
        coverageList.forEach((range) => {
            addCssCoverage(coverageInfo, range, locator);
        });
    }
...
const handleOriginalBranchesCoverage = (state, originalMap) => {
    ...
    state.astInfo.branches.forEach((it) => {
        ...
        const newLocations = locations.map((oLocation) => {

            const {
                start, end, count
            } = oLocation;

            // no start/end for else branch
            if (!start || !end) {
                return {
                    count
                };
            ...
            if (!res) {
                // should be failed if no found
                Util.logError(`Not found original branch. start: ${oLocation.start}, end: ${oLocation.end}`);
                return {
                    count
                };
            }
    ...
    if (js) {
        ...
        coverageList.forEach((block) => {
            block.ranges.forEach((range, index) => {
                ...
                const { originalRange, originalState } = result;
                addJsCoverage(originalState, originalRange);
