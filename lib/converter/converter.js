...
const initFileCoverage = (positionMapping, type, sourcePath) => {
    ...
    const lineMap = {};
    const lines = [];
    const { blankLines, commentedLines } = positionMapping;
    positionMapping.lines.forEach((it) => {
        // exclude comments and blanks
        if (blankLines.includes(it.line) || commentedLines.includes(it.line)) {
            return;
        }
        // line 1-base
...
const calculateV8Lines = (lines, blankLines, commentedLines) => {
    const v8Lines = {
        total: 0,
        covered: 0,
        blank: blankLines.length,
        commented: commentedLines.length
    };
...
const getFileCoverage = (coverage, sourcePath) => {
    ...
    const v8Coverage = {
        ...
        summary: {
            ...
            lines: calculateV8Lines(lines, blankLines, commentedLines)
        }
    };
...
const getDistCoverage = (item, state) => {
    ...
    const { type, sourcePath } = item;
    if (type === 'js') {
        const coverage = initFileCoverage(positionMapping, type, sourcePath);
...
const initOriginalList = (sourceMap, originalIndexMap, fileSources, options) => {
    ...
    sources.forEach((sourcePath, sourceIndex) => {
        ...
        const coverage = initFileCoverage(positionMapping, 'js', sourcePath);
