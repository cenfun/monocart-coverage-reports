...
const { mergeIstanbulCoverage } = require('../istanbul/istanbul.js');
 ...
 */
const collectFileCoverage = (item, state, options) => {
    ...
    // save istanbul data
    state.istanbulData = istanbulData;
...
const collectOriginalList = (item, state, originalMap, options) => {
    ...
    originalMap.forEach((originalState) => {
        ...
        collectFileCoverage(sourceItem, originalState, options);
...
const generateCoverageForDist = (item, state, options) => {
    ...
    collectFileCoverage(item, state, options);
...
const unpackSourceMap = async (item, state, options) => {
    ...
    collectOriginalList(item, state, originalMap, options);
...
const mergeV8Data = (prevV8Data, v8Data) => {
    const sourcePath = prevV8Data.sourcePath;

    console.log('merge v8 data ===================================', sourcePath);

    const mergedV8Data = v8Data;

    return mergedV8Data;
};

const mergeIstanbulData = (prevIstanbulData, istanbulData) => {

    const sourcePath = prevIstanbulData.path;

    // merge istanbul data
    const prevIstanbulCoverage = {};
    prevIstanbulCoverage[sourcePath] = prevIstanbulData;
    // if (sourcePath.endsWith('component.js')) {
    //     console.log(prevIstanbulData.path, prevIstanbulData.s);
    // }

    const istanbulCoverage = {};
    istanbulCoverage[sourcePath] = istanbulData;
    // if (sourcePath.endsWith('component.js')) {
    //     console.log(istanbulData.path, istanbulData.s);
    // }

    const mergedIstanbulCoverage = mergeIstanbulCoverage([prevIstanbulCoverage, istanbulCoverage]);
    const mergedIstanbulData = mergedIstanbulCoverage[sourcePath];
    // if (sourcePath.endsWith('component.js')) {
    //     console.log(mergedIstanbulData.path, mergedIstanbulData.s);
    // }

    return mergedIstanbulData;
};

const generateV8DataList = (stateList) => {
    ...
    stateList.forEach((state) => {
        const {
            v8Data, istanbulData, originalList
        } = state;
        // dedupe dist file if not debug
        if (!v8Data.dedupe) {
            dataMap.set(v8Data.id, {
                v8Data,
                istanbulData
            });
        }
    ...
    allOriginalList = allOriginalList.flat();

    // merge istanbul and v8(converted)
    allOriginalList.forEach((originalState) => {

        const { v8Data, istanbulData } = originalState;

        // exists item
        const prevData = dataMap.get(v8Data.id);
        if (prevData) {
            // ignore empty item, just override it
            if (!prevData.v8Data.empty) {
                prevData.v8Data = mergeV8Data(prevData.v8Data, v8Data);
                prevData.istanbulData = mergeIstanbulData(prevData.istanbulData, istanbulData);
                return;
            }
        }
    ...
    dataMap.forEach((dataItem) => {
        const { v8Data, istanbulData } = dataItem;
...
const convertV8List = async (v8list, options) => {
    ...
    return generateV8DataList(stateList);
