...
const { dedupeRanges, dedupeCountRanges } = require('../utils/dedupe.js');
...
const initIgnoredRanges = (state, options) => {
    if (!options.enableV8Ignore) {
        return;
    }

    const ignoredRanges = getIgnoredRanges(state.locator);
    if (ignoredRanges) {
        state.coverageInfo.ignoredRanges = ignoredRanges;
    }
};
...
const handleIgnoredLines = (lineMap, locator, ignoredRanges) => {
 ...
 */
const collectFileCoverage = (item, state, coverageData) => {
    ...
    if (ignoredRanges) {
        // ranges for line in UI after formatted
        const ignores = ignoredRanges.map((it) => {
            return {
                start: it.startOffset,
                end: it.endOffset
            };
        });
        data.ignores = dedupeRanges(ignores);
        ...
        handleIgnoredLines(lineMap, locator, ignoredRanges);
...
const generateCoverageForDist = (item, state, options) => {
    ...
    initIgnoredRanges(state, options);

    handleFunctionsCoverage(state);
    handleBranchesCoverage(state);

    initLinesCoverage(state);
    handleBytesCoverage(state);

    collectFileCoverage(item, state, state.coverageData);
...
const collectOriginalList = (item, state, originalMap) => {
    ...
    originalMap.forEach((originalState) => {
        ...
        collectFileCoverage(sourceItem, originalState, state.coverageData);
