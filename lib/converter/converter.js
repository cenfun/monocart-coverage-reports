...
const initOriginalList = (state, originalDecodedMap, options) => {
    ...
    const lengthBefore = sourceList.length;
    sourceList = sourceList.filter(sourceFilter);
    const lengthAfter = sourceList.length;
    Util.logFilter(`source filter (${state.sourcePath}):`, lengthBefore, lengthAfter);

    sourceList.forEach((sourcePath, sourceIndex) => {
