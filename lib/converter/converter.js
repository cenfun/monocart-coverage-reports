 ...
 */
const getFileCoverage = (sourcePath, coverage) => {
    ...
    coverage.lines.forEach((line, index) => {
    ...
    coverage.functions.forEach((fn, index) => {
...
const unpackDistSource = (item, state) => {
    ...
    const { v8Coverage, istanbulCoverage } = getFileCoverage(sourcePath, coverage);
