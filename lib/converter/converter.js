...
const mergeV8Data = (state, stateList) => {
    ...
    Object.keys(uncoveredMap).forEach((key) => {
        for (const group of uncoveredGroups) {
            if (!group[key]) {
                return;
            }
        }
        const range = uncoveredMap[key];
        // the uncovered range should be in all uncovered ranges
        // but not for remix tree shacking, there is no code and coverage data in client side, will be covered by default
        mergedBytes.push(range);
    });
