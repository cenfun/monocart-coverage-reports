...
const initIgnoredRanges = (state, options) => {
    ...
    if (ignoredRanges) {
        state.ignoredRanges = ignoredRanges;
    }
};

const handleIgnoredRanges = (list, state, options) => {
    if (!options.enableV8Ignore) {
        return;
    }

    const ignoredRanges = state.ignoredRanges;
    if (!ignoredRanges) {
        return;
    }
...
const handleFunctionsCoverage = (state, options) => {
    ...
    handleIgnoredRanges(v8Functions, state, options);
...
const handleBranchesCoverage = (state, options) => {
    ...
    handleIgnoredRanges(v8Branches, state, options);
...
const generateCoverageForDist = (item, state, options) => {
    ...
    handleFunctionsCoverage(state, options);
    handleBranchesCoverage(state, options);
...
const unpackSourceMap = async (item, state, options) => {
    ...
    originalMap.forEach((originalState) => {
        ...
        handleFunctionsCoverage(originalState, options);
        handleBranchesCoverage(originalState, options);
