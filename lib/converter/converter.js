...
const getDistAstInfo = (item, positionMapping) => {
...
const handleLinesCoverage = (state) => {
    ...
    const {
        type, positionMapping, coverage, astInfo
    } = state;
...
const generateCoverageForDist = (item, state) => {
    ...
    const { v8Coverage, istanbulCoverage } = getFileCoverage(state.coverage, sourcePath);
...
const convertV8List = async (v8list, options) => {
    ...
    for (const item of v8list) {
        ...
        const state = {
            ...
            positionMapping,
            coverage: createEmptyCoverage(),
            astInfo: getDistAstInfo(item, positionMapping)
        };
