...
const handleIgnoredRanges = (state, options) => {
    state.ignoredRanges = getIgnoredRanges(state.locator, options);
};
...
const generateCoverageForDist = (item, state, options) => {
    ...
    // handle coverage
    handleIgnoredRanges(state, options);
...
const unpackSourceMap = async (item, state, options) => {
    ...
    originalMap.forEach((originalState) => {
        handleIgnoredRanges(originalState, options);
