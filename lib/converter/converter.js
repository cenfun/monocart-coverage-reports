...
const isWrapFunction = (positionMapping, start, end) => {
    ...
    // end must be empty
    if (Util.isBlank(right)) {
        // console.log('=================================', state.sourcePath);
        if (Util.isBlank(left)) {
            return true;
        }

        // special case, left is export
        const leftStr = left.trim();

        const specialList = ['export', '"use strict";', "'use strict';"];
        if (specialList.includes(leftStr)) {
            return true;
        }
...
const handleLinesCoverage = (state) => {
    ...
    const {
        js, positionMapping, coverageInfo
    } = state;
