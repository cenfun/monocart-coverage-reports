...
const handleBranchesCoverage = (state) => {
    ...
    astInfo.branches.forEach((it, i) => {
        ...
        const locations = it.locations.map((l) => {
            const sl = locator.offsetToLocation(l.start);
            const el = locator.offsetToLocation(l.end);
            return {
                start: {
                    line: sl.line,
                    column: sl.column
        ...
        const branchInfo = new InfoBranch(branchType, startLine, branchLoc, locations);
...
const handleOriginalBranchesCoverage = (state, originalMap) => {
    ...
    state.astInfo.branches.forEach((it) => {
        ...
        const newLocations = locations.map((location) => {
            ...
            if (!res) {
                Util.logError('Not found branch location original range');
                return;
            }
