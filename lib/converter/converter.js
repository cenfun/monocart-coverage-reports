...
const initFileCoverage = (positionMapping, type, sourcePath) => {
    ...
    const lineMap = {};
    const lines = [];
    const { blankLines, commentLines } = positionMapping;
    positionMapping.lines.forEach((it) => {
        // exclude comments and blanks
        if (blankLines.includes(it.line) || commentLines.includes(it.line)) {
            return;
        }
        // line 1-base
...
const calculateV8Lines = (lines, blankLines, commentLines) => {
    const v8Lines = {
        total: 0,
        covered: 0,
        blank: blankLines.length,
        comment: commentLines.length
    };
