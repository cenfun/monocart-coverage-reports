...
const getAstInfo = (item, coverageList) => {
    ...
    if (type === 'js') {
        ...
        //     // }
        //     console.log(range, functionName);
...
const updateFunctionsCount = (coverageInfo, sLoc, eLoc, count, functionName) => {
    if (!count) {
        return;
    }

    const { functionMap, functions } = coverageInfo;
...
const addJsCoverage = (coverageInfo, block, range, index, positionMapping) => {
    ...
    if (isBlockCoverage) {
        ...
        if (index === 0) {
            ...
            updateFunctionsCount(coverageInfo, sLoc, eLoc, count, functionName);
        }
...
const handleFunctionsCoverage = (state) => {
    ...
    const functionMap = coverageInfo.functionMap;
...
const handleLinesCoverage = (state) => {
    ...
    const {
        type, positionMapping, coverageInfo, astInfo
    } = state;
    ...
    const baseLineCount = type === 'js' ? 1 : 0;
...
const handleRangesCoverage = (state) => {
    ...
    const {
        type, positionMapping, coverageInfo, coverageList
    } = state;
...
const collectOriginalList = (item, state, originalMap) => {
    ...
    originalMap.forEach((originalState) => {
        const {
            source, type, sourcePath, coverageInfo
        } = originalState;
