...
const { dedupeCountRanges, mergeRangesWith } = require('../utils/dedupe.js');
...
const unpackSourceMap = async (state, options) => {

    const { sourcePath, sourceMap } = state;
...
const unpackDistFile = async (item, state, options) => {
    ...
    if (state.sourceMap) {
        ...
        // unpack source map
        await unpackSourceMap(state, options);
...
const mergeV8Data = (state, stateList) => {
    ...
    const mergedFunctions = dedupeCountRanges(allFunctions);
    ...
    const mergedBranches = mergeRangesWith(allBranches, (lastRange, range) => {
