...
// for debug
const logMappingErrors = (type, errors) => {
    // console.log(`${Util.EC.red('[mapping error]')} ${Util.EC.magenta(type)} ${errors.join(' -> ')}`);
};
...
const handleOriginalFunctionsCoverage = (state, originalStateMap) => {
    ...
    const updateFunctionBodyRange = (start, end, bodyStart, bodyEnd, originalFunction) => {
        if (bodyStart !== start || bodyEnd !== end) {
            ...
            if (result.error) {
                logMappingErrors('function body', result.errors);
            } else {
    ...
    state.astInfo.functions.forEach((it) => {
        ...
        if (result.error) {
            logMappingErrors('function', result.errors);
            return;
        }
...
const handleOriginalBranchesCoverage = (state, originalStateMap) => {
    ...
    state.astInfo.branches.forEach((group) => {
        ...
        if (result.error) {
            logMappingErrors('branch group', result.errors);
            return;
        }
        ...
        const newLocations = locations.map((oLoc) => {
            ...
            if (locResult.error) {
                ...
                logMappingErrors('branch', locResult.errors);
                hasError = true;
                return newLoc;
            }
...
const handleOriginalStatementsCoverage = (state, originalStateMap) => {
    ...
    state.astInfo.statements.forEach((it) => {
        ...
        const result = findOriginalRange(start, end, state, originalStateMap);
        if (result.error) {
            logMappingErrors('statement', result.errors);
            return;
        }
    ...
    if (js) {
        ...
        coverageList.forEach((block) => {
            block.ranges.forEach((range, index) => {
                ...
                if (result.error) {
                    logMappingErrors('byte', result.errors);
                    return;
                }
