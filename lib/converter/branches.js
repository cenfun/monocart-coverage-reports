...
const findBranchBlock = (start, end, functionRange) => {
    if (functionRange) {
...
const addBranch = (group, node, locationMap, functionInfo) => {
    ...
    const block = findBranchBlock(start, end, functionRange);
    ...
    const branchInfo = {
        ...
        // cache block
        block,
        // branch count default to 0 if no function range, function count is 1
        count: functionRange ? 0 : 1
    };
...
const addNoneBranch = (group, functionInfo) => {
    const functionRange = functionInfo.range;
...
const addNodeCount = (node, parents, functionInfo) => {
    ...
    if (functionRange) {
        ...
        // default to 1 if no function range
        node._state = {
            count: 1
        };
        return;
    ...
    Util.visitAst(ast, {
        ...
        IfStatement: (node, parents) => {
            ...
            if (alternate) {
                ...
                // add none branch
                addNoneBranch(group, functionInfo);
...
const LogicalExpression = (group, parentCount, noBlockList, blockCount) => {
    ...
    //     console.log(group.type, parentCount, 'locations', group.locations, group.functionInfo.range);
    // }
