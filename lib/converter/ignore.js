...
const addNextIgnore = (list, content, item, locator) => {
    ...
    let end = endLine ? endLine.end : maxLength;
    // extend right
    while (end < maxLength && Util.isBlank(source[end + 1])) {
        end += 1;
    }
...
const hasIgnoreKey = (content) => {
    ...
    const istanbulKey = 'istanbul ignore';
    if (content.startsWith(istanbulKey)) {
        return istanbulKey.length;
    }
};
