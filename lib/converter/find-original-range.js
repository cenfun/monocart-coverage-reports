...
const getCommonDiffPosition = (gt, gp, ot, direction) => {
...
const getCommonFixedPosition = (info, direction) => {
    ...
    const multipleLinePos = getCommonDiffPosition(generatedText, generatedPos, originalText, direction);
    if (multipleLinePos) {
        return multipleLinePos;
    }

    // never cross line, using first line
    // trim end remove \r\n and \n
    const gLines = generatedText.split(/\n/);
    const oLines = originalText.split(/\n/);

    if (gLines.length === 1 && oLines.length === 1) {
        // already single line
        info.generatedLine = generatedText;
        info.originalLine = originalText;

        return;
    }

    const generatedLine = gLines[0].trimEnd();
    const originalLine = oLines[0].trimEnd();
    ...
    if (generatedPos > generatedLine.length) {
        return;
    }

    const firstLinePos = getCommonDiffPosition(generatedLine, generatedPos, originalLine, direction);
    if (firstLinePos) {
        return firstLinePos;
    }
...
const getFixedStartPosition = (info) => {
    ...
    const blockPos = getBlockStartPosition(info);
    if (blockPos) {
        return blockPos.pos;
    }
...
const getFixedEndPosition = (info) => {
    ...
    const blockPos = getBlockEndPosition(info);
    if (blockPos) {
        return blockPos.pos;
    }
...
const getOriginalEndOffset = (m, originalState) => {
    if (!m.originalEndOffset) {
        ...
        // last column
        const toEndLength = line.length - m.originalColumn;
        m.originalEndOffset = m.originalOffset + toEndLength;
    }
    return m.originalEndOffset;
};

const getOriginalText = (m1, m2, originalState) => {
    const originalLocator = originalState.locator;

    // esbuild fixing
    if (m1.originalOffset === m2.originalOffset) {
        const originalEndOffset = getOriginalEndOffset(m1, originalState);
        return originalLocator.getSlice(m1.originalOffset, originalEndOffset);
    }

    return originalLocator.getSlice(m1.originalOffset, m2.originalOffset);
};
...
const getFixedOriginalStart = (start, mappings, state, originalResult) => {
    ...
    if (m1.originalOffset > m2.originalOffset) {
        return {
            error: true,
            errors: [`invalid original start offsets: ${EC.yellow(m1.originalOffset)} > ${EC.yellow(m2.originalOffset)}`]
        };
    }

    const originalText = getOriginalText(m1, m2, originalState);

    const generatedLocator = state.locator;
    const generatedText = generatedLocator.getSlice(m1.generatedOffset, m2.generatedOffset);
    const generatedPos = start - m1.generatedOffset;
    ...
    return {
        originalStart: m1.originalOffset + originalPos
    };
};
...
const getFixedOriginalEnd = (end, mappings, state, originalResult) => {
    ...
    if (m1.originalOffset > m2.originalOffset) {
        return {
            error: true,
            errors: [`invalid original end offsets: ${EC.yellow(m1.originalOffset)} > ${EC.yellow(m2.originalOffset)}`]
        };
    }
    ...
    const generatedLocator = state.locator;
    const generatedText = generatedLocator.getSlice(m1.generatedOffset, m2.generatedOffset);
    const generatedPos = end - m1.generatedOffset;
    ...
    return {
        originalEnd: m1.originalOffset + originalPos
    };
};
