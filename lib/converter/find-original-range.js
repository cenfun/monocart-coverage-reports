...
const getOriginalStartPosition = (start, startMapping, state, rangeInfo) => {
...
const getOriginalEndPosition = (end, endMapping, state, rangeInfo) => {
    if (Array.isArray(endMapping)) {
        return getFixedOriginalEnd(end, endMapping, state, rangeInfo);
    }
    // Exact match, but end is exclusive, so it could be wrong original end
    // TODO
    return {
...
const findOriginalRange = (start, end, state, originalMap) => {
    ...
    const originalStartResult = getOriginalStartPosition(start, startMappings, state, rangeInfo);
    if (originalStartResult.error) {
        return createMappingError(originalStartResult.errors);
    }

    const originalEndResult = getOriginalEndPosition(end, endMappings, state, rangeInfo);
