...
const getOriginalStartPosition = (rangeInfo, state) => {

    const { start, startMappings } = rangeInfo;

    if (Array.isArray(startMappings)) {
        return getFixedOriginalStart(start, startMappings, state, rangeInfo);
    }
    // Exact match
    return {
...
const getOriginalEndPosition = (rangeInfo, state) => {

    const { end, endMappings } = rangeInfo;
    ...
    if (Array.isArray(endMappings)) {
        ...
        // end, exclusive
        const newMappings = findMapping(decodedMappings, end);
        if (Array.isArray(newMappings)) {
            return getFixedOriginalEnd(end, newMappings, state, rangeInfo);
        }

        // const [m1, m2] = endMappings;
        // // always m2 === newMappings
        // if (m2 !== newMappings) {
        //     console.log([m1, m2, newMappings]);
        // }

        // Exact match end
        return {
            originalEnd: newMappings.originalOffset
        };
    ...
    // Exact match (end - 1)
    return {
        originalEnd: endMappings.originalOffset + 1
    };
};
...
const getOriginalState = (start, end, originalMap, state) => {
...
const findOriginalRange = (start, end, state, originalMap) => {
    ...
    const rangeInfo = getOriginalState(start, end, originalMap, state);
    if (rangeInfo.error) {
        return createMappingError(rangeInfo.errors);
    }

    const originalStartResult = getOriginalStartPosition(rangeInfo, state);
    if (originalStartResult.error) {
        return createMappingError(originalStartResult.errors);
    }

    const originalEndResult = getOriginalEndPosition(rangeInfo, state);
