...
const getOriginalText = (m1, m2, rangeInfo) => {
    ...
    if (o1 < o2) {
        ...
        return originalLocator.getSlice(o1, o2);
    }
    ...
    if (o1 === o2) {
        ...
        return originalLocator.getSlice(o1, originalEndOffset);
    }

    // o1 > o2
    return originalLocator.getSlice(o2, o1);
...
const checkOriginalMappings = (m1, m2, direction) => {
    // check original offset order
    ...
    return {
        error: true,
        errors: [`invalid original ${direction}: ${EC.yellow(m1.originalOffset)} > ${EC.yellow(m2.originalOffset)}`]
    };
};
...
const getFixedOriginalStart = (start, mappings, state, rangeInfo) => {
    ...
    const errorInfo = checkOriginalMappings(m1, m2, direction);
    if (errorInfo) {
        return errorInfo;
    }

    const originalText = getOriginalText(m1, m2, rangeInfo);
